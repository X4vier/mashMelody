...
const PAD_CHORDS = ["A4_C5", "C4_E4", "F4_A4", "G4_B4"].map((chord) => `pad_pad_chord_${chord}.wav`);
const allSounds = [
    ...ALL_NOTES.map((note) => ({
        type: "note",
        sound: note,
...
function getStableMapping(key) {
    ...
    const index = KEY_SOUND_MAPPING.findIndex((k) => k.toLowerCase() === key.toLowerCase());
    if (index === -1) {
...
export class SoundEffect {
    constructor() {
        ...
        this.activeChords = [];
        this.currentProgression = Math.floor(Math.random() * CHORD_PROGRESSIONS.length);
    ...
    async loadSounds() {
        ...
        for (const chord of PAD_CHORDS) {
            try {
                const response = await fetch(`./sounds/${chord}`);
                if (!response.ok) {
                    console.warn(`Failed to load pad chord ${chord}: ${response.statusText}`);
                    continue;
                }
                const arrayBuffer = await response.arrayBuffer();
                const audioBuffer = await this.audioContext.decodeAudioData(arrayBuffer);
    ...
    playSound(sound) {
        ...
        if (buffer) {
            ...
            if (sound.startsWith("chord_")) {
                ...
                const activeChordsToFade = this.activeChords.filter((chord) => !chord.fadingOut);
                activeChordsToFade.forEach((chord) => {
                    chord.fadingOut = true;
                    chord.gain.gain.setValueAtTime(chord.gain.gain.value, this.audioContext.currentTime);
                    chord.gain.gain.linearRampToValueAtTime(0, this.audioContext.currentTime + CHORD_FADE_DURATION_MS / 1000);
                    setTimeout(() => {
                        chord.source.stop();
    ...
    async handleKeyPress(e) {
        ...
        if (mapping.type === "note") {
            ...
            const nextChord = CHORD_PROGRESSIONS[this.currentProgression][this.currentChordIndex];
