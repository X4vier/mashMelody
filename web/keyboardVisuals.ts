...
import {
  keyHorizontalPositions,
  keyVerticalPositions,
  keyEmojis,
} from "./keyPositions.js";
...
interface Particle {
  ...
  life: number;
  maxLife: number;
  prevPosition: Point; // Track previous position for motion blur
}
...
export class KeyboardVisuals {
  ...
  private readonly particlesPerPress = 15;
  private readonly minX: number;
  private readonly maxX: number;
  private readonly minY: number;
  private readonly maxY: number;
  private readonly maxEmojiSize = 300;
  private readonly emojiLifetime = 1500;
  ...
  public handleKeyPress(e: KeyboardEvent): void {
    ...
    const rawXPosition =
      keyHorizontalPositions[key as keyof typeof keyHorizontalPositions] ?? 0.5;
    const rawYPosition =
      keyVerticalPositions[key as keyof typeof keyVerticalPositions] ?? 0.5;
    ...
    const globalOffsetRange =
      Math.max(this.ctx.canvas.width, this.ctx.canvas.height) * 0.1; // 10% of max canvas dimension
    ...
    const emoji = keyEmojis[key as keyof typeof keyEmojis];
    if (emoji) {
  ...
  public loop(): void {
    ...
    this.emojis = this.emojis.filter(
      (emoji) => now - emoji.createdAt < this.emojiLifetime
    );
  }
